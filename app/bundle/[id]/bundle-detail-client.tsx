...
/**
 * BundleDetailClient — interactive shell for the bundle detail page.
 *
 * Receives pre-fetched bundle data from the Server Component (page.tsx).
 * Handles: add-all-to-cart, cart panel.
 */
...
import type { Bundle, BundleProduct } from '@/lib/api/types';
  ...
  return (
    <div className="min-h-screen bg-[#fff0f5] font-nunito">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
          {bundle.productIds.map((p: BundleProduct) => (
            ...
            >
              <Link
                href={`/product/${p._id}`}
                className="block relative aspect-square bg-linear-to-br from-pink-50 to-rose-50 overflow-hidden"
              >
                {p.images?.[0] ? (
                  <Image
                    src={p.images[0]}
                ...
                {p.badge && (
                  <span className="absolute top-2 left-2 bg-linear-to-r from-[#e91e8c] to-[#c2185b] text-white text-[10px] font-extrabold px-2 py-0.5 rounded-full">
                    {p.badge}
                  </span>
                )}
              </Link>
        ...
        {/* Savings callout */}
        {bundle.originalPrice && bundle.originalPrice > bundle.bundlePrice && (
          <div className="mt-10 bg-linear-to-r from-[#fce4ec] to-white rounded-2xl border border-pink-200 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
            ...
            <button
              ...
              className={`shrink-0 inline-flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm transition-all ${
                added
                  ? 'bg-green-500 text-white'
                  : 'bg-linear-to-r from-[#e91e8c] to-[#c2185b] text-white hover:shadow-lg hover:shadow-[#e91e8c]/30 hover:-translate-y-0.5'
              }`}
            >
