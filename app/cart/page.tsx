...
} from '@/lib/hooks/use-shipping-settings';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {items.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h2 className="font-playfair text-2xl font-bold text-[#2d1a24] mb-3">
              Your cart is empty
            </h2>
            ...
            <Link
              href="/"
              className="bg-linear-to-r from-[#e91e8c] to-[#c2185b] text-white px-8 py-3 rounded-full font-bold text-sm hover:shadow-lg transition-all inline-block"
            >
              Continue Shopping
            </Link>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
              ...
              {items.map((item) => (
                ...
                >
                  <Link
                    ...
                    className="relative w-24 h-24 sm:w-28 sm:h-28 bg-pink-50 rounded-xl shrink-0 overflow-hidden"
                  >
                    {item.image ? (
                      <Image
                        src={item.image}
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Package className="w-10 h-10 text-[#ad1457]/40" />
                      </div>
                    )}
                  </Link>
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <Link href={`/product/${item.productId}`}>
                          <h3 className="font-playfair text-lg font-bold text-[#2d1a24] hover:text-[#e91e8c] transition-colors">
                            {item.name}
                          </h3>
                        </Link>
                        {item.variantLabel && (
                          <p className="text-xs text-[#ad1457] mt-0.5">{item.variantLabel}</p>
                        )}
                        {item.sku && (
                          <p className="text-[10px] text-[#6d1b3b]/40 mt-0.5">SKU: {item.sku}</p>
                        )}
                      </div>
                      <button
                        onClick={() => removeItem(item.productId, item.variantId)}
                        className="text-[#ad1457]/50 hover:text-red-500 transition-colors"
                        aria-label="Remove"
                      >
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() =>
                            updateQty(item.productId, item.qty - 1, item.variantId)
                          }
                        ...
                        <span className="font-bold text-sm text-[#2d1a24] w-6 text-center">
                          {item.qty}
                        </span>
                        <button
                          onClick={() =>
                      ...
                      <div className="text-right">
                        {item.originalPrice && item.originalPrice > item.price && (
                          <span className="font-nunito text-xs text-[#ad1457] line-through mr-2">
            ...
            <div className="lg:w-90 shrink-0">
              <div className="bg-white rounded-2xl border-2 border-[#e91e8c]/20 p-6 sticky top-20">
                <h3 className="font-playfair text-xl font-bold text-[#2d1a24] mb-5">
                  Order Summary
                </h3>
                <div className="space-y-3 mb-5">
                  <div className="flex justify-between text-sm">
                    <span className="text-[#2d1a24]/70">Subtotal ({itemCount} items)</span>
                    <span className="font-semibold text-[#2d1a24]">
                  ...
                  {shipping > 0 && (
                    <p className="text-[10px] text-[#ad1457]">
                ...
                <button
                  ...
                  className="w-full bg-linear-to-r from-[#e91e8c] to-[#c2185b] text-white py-4 rounded-full font-bold text-base hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#e91e8c]/35 transition-all"
                >
                  PROCEED TO CHECKOUT ✨
                </button>
