...
import { useRouter } from "next/navigation";
import Link from "next/link";
import { ShoppingBag, Package, CreditCard, Smartphone, Phone, MapPin, Home, Truck, Zap, Check, Lock, ArrowLeft, PartyPopper } from "lucide-react";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { useCart } from "@/lib/cart-context";
import { useAuth } from "@/lib/auth-context";
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            <div className="mb-8">
              ...
              <div className="space-y-3">
                {[
                  { id: "cod" as const, label: "Cash on Delivery", icon: CreditCard, desc: "Pay when you receive" },
                  { id: "bkash" as const, label: "bKash", icon: Smartphone, desc: "Mobile payment" },
                  { id: "nagad" as const, label: "Nagad", icon: Phone, desc: "Mobile payment" },
                ].map((method) => (
                  <button key={method.id} onClick={() => setPaymentMethod(method.id)}
                    className={`w-full p-4 rounded-xl border-2 text-left flex items-center gap-4 transition-all cursor-pointer ${paymentMethod === method.id ? "border-[#e91e8c] bg-[#e91e8c]/5" : "border-[#fce4ec] bg-white hover:border-[#e91e8c]/30"}`}>
                    <method.icon className="w-6 h-6 text-[#ad1457]" />
                    <div>
                      <div className="font-bold text-sm text-[#2d1a24]">{method.label}</div>
                      <div className="text-xs text-[#ad1457]">{method.desc}</div>
                    </div>
                    {paymentMethod === method.id && <Check className="ml-auto text-[#e91e8c] w-5 h-5" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="lg:w-[360px] shrink-0">
            <div className="bg-white rounded-2xl border-2 border-[#e91e8c]/20 p-6 sticky top-20">
              ...
              <p className="text-center text-[10px] text-[#ad1457] mt-3 flex items-center justify-center gap-1">
                <Lock className="w-3 h-3" /> Your information is secure
              </p>
            </div>
          </div>
        </div>
