...
export default function ProductsPage() {
  ...
  const getStatusInfo = (product: Product) => {
    const totalStock = product.variants?.reduce((sum, v) => sum + v.stockQuantity, 0) || 0;
    if (totalStock === 0) return { label: "Out of Stock", color: "bg-red-100 text-red-700", icon: AlertCircle };
    if (totalStock <= 10) return { label: "Low Stock", color: "bg-yellow-100 text-yellow-700", icon: AlertCircle };
    return { label: "Active", color: "bg-green-100 text-green-700", icon: BadgeCheck };
  };
          ...
          {products.map((product) => {
            ...
            return (
              <div key={product._id} className="bg-white rounded-2xl border border-pink-100 p-5 hover:shadow-md hover:shadow-pink-100/50 transition-all group relative">
                ...
                <h3 className="text-sm font-bold text-[#2d1a24] mb-1 line-clamp-1">{product.name}</h3>
                <p className="text-xs text-[#6d1b3b]/50 mb-3 flex items-center gap-1">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-[#6d1b3b]/50 flex items-center gap-1">
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-pink-50">
                  <span className="text-xs text-[#6d1b3b]/50 flex items-center gap-1">
                    ...
                    {product.salesCount} sold • {product.variants?.length || 0} variant{(product.variants?.length || 0) !== 1 ? 's' : ''}
                  </span>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      onClick={() => openEdit(product)} 
                      className="text-xs text-[#e91e8c] font-semibold hover:text-[#ad1457] flex items-center gap-1"
                    >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button 
            ...
            disabled={page === 1}
            className="px-3 py-2 rounded-lg border border-pink-200 text-sm text-[#6d1b3b] disabled:opacity-40 hover:bg-pink-50 transition-all flex items-center gap-1"
          >
            ← Prev
          </button>
          <span className="text-sm text-[#6d1b3b]/60 px-3">Page {page} of {totalPages}</span>
          ...
          <div className="bg-white rounded-2xl border border-pink-100 shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-pink-100 px-6 py-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-[#2d1a24] font-playfair flex items-center gap-2">
                {modalMode === "create" ? <Plus className="w-5 h-5 text-[#e91e8c]" /> : <Edit className="w-5 h-5 text-[#e91e8c]" />}
                {modalMode === "create" ? "Create Product" : "Edit Product"}
              </h2>
              <button 
                onClick={closeModal} 
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#2d1a24] mb-1.5 flex items-center gap-1">
                      Original Price <span className="text-xs text-[#6d1b3b]/50">(optional)</span>
                    </label>
                    <input 
                      type="number" 
                ...
                <div className="border-t border-pink-100 pt-4">
                  <div className="flex items-center justify-between mb-3">
                    <label className="text-sm font-medium text-[#2d1a24] flex items-center gap-2">
                    ...
                    {modalMode === "create" && (
                      <button 
                        ...
                        {formData.useVariants ? "Multiple Variants Enabled" : "Enable Variants"}
                      </button>
                    )}
                  </div>
                  ...
                  {!formData.useVariants && modalMode === "create" && (
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full px-4 py-2.5 rounded-xl border border-pink-200 text-sm text-[#2d1a24] outline-none focus:border-[#e91e8c] focus:ring-2 focus:ring-[#e91e8c]/10 transition-all placeholder:text-[#ad1457]/30" 
                      />
                      <p className="text-[10px] text-[#6d1b3b]/40 mt-1 flex items-center gap-1">
                        <Settings className="w-3 h-3" />
                        A default variant will be auto-created with this stock
                      </p>
                    </div>
                  )}
                    ...
                    <div className="space-y-3">
                      {formData.variants.map((variant, idx) => (
                        <div key={idx} className="bg-gradient-to-br from-pink-50/50 to-white rounded-xl p-4 border border-pink-100">
                          ...
                          <button 
                            ...
                            className="text-[10px] text-[#e91e8c] font-medium mt-2 hover:underline flex items-center gap-1"
                          >
                            <PlusCircle className="w-3 h-3" /> Add Attribute (Size, Color, etc.)
                          </button>
                        </div>
                      ))}
                      ...
                      {modalMode === "create" && (
                        <button 
                          ...
                          className="w-full py-2 rounded-xl border border-dashed border-pink-200 text-xs text-[#e91e8c] font-semibold hover:bg-pink-50 transition-all flex items-center justify-center gap-1"
                        >
