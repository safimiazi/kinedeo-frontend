...
/**
 * CategoryProductsClient — handles infinite scroll, add-to-cart, and wishlist.
 * Receives initial SSR data from CategoryPage (page.tsx).
 */
...
export default function CategoryProductsClient({
  ...
  // slug comes from the URL — still needed for the category nav highlight
  ...
  // Infinite scroll query — starts from page 2 since we have page 1 from SSR
  const {
    data,
    fetchNextPage,
    hasNextPage,
  ...
  // Merge SSR initial data with React Query pages
  // React Query's first page duplicates SSR data, so we show SSR products
  // until React Query has loaded, then switch to RQ data entirely
  const rqProducts = data?.pages.flatMap((p) => p.products) ?? [];
  const products = rqProducts.length > 0 ? rqProducts : initialProducts;
  ...
  return (
    <div className="min-h-screen bg-[#fff0f5] font-nunito">
      ...
      <div className="relative bg-linear-to-r from-[#e91e8c] via-[#c2185b] to-[#ad1457] text-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
          ...
          <div className="flex items-start gap-4">
            {category.image ? (
              <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-white/30 shadow-lg shrink-0 relative">
                <Image
                  src={category.image}
                  alt={category.name}
                  fill
            ...
            <div>
              ...
              <p className="text-white/60 text-sm mt-1.5 font-semibold">{total} products</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Empty */}
        {!isFetchingMore && !isError && products.length === 0 && (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative aspect-square bg-linear-to-br from-pink-50 to-rose-50 overflow-hidden">
                      ...
                      <button
                        onClick={() =>
                          toggleFavorite.mutate({ productId: p._id, isFavorited: isFav })
                        }
                      ...
                      >
                        <Heart
                          className={`w-3.5 h-3.5 ${isFav ? 'fill-white text-white' : 'text-[#ad1457]'}`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="p-3 flex flex-col flex-1">
                      <div className="flex items-center gap-1 mb-1.5">
                        ...
                        {p.reviewCount > 0 && (
                          <span className="text-[10px] text-[#6d1b3b]/40 ml-0.5">
                            ({p.reviewCount})
                          </span>
                        )}
                      </div>
                      ...
                      <p className="text-[11px] text-[#6d1b3b]/50 line-clamp-1 mb-2">
                        {p.shortDescription}
                      </p>
        ...
        <div ref={loadMoreRef} className="mt-8 flex justify-center py-4">
          ...
          {!hasNextPage && products.length > 0 && !isFetchingMore && (
